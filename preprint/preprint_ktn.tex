...
$P_{\mathrm{sig}}$ drifts by $1.8\,\%$ (Table~\ref{tab:regen}). The
...
penalizing what noise creates (disorder). The outcome on
\texttt{ibm\_fez} is a positive separation between the woven-type and
aligned-type states (full numbers in the repository artefacts
\texttt{ktn\_phase5\_qpu.json}), consistent with the exact-simulation
contrast of $0.336$ and with the earlier four-qubit validation contrast of
$0.337$ on the same backend \cite{repo}.
